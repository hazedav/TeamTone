    ...
    def test_filament_structure(self, filaments):
        ...
        for filament in filaments[:10]:  # Check first 10
            for field in required_fields:
                assert field in filament, f"Missing field '{field}' in filament: {filament}"
                assert filament[field], f"Empty value for field '{field}' in filament: {filament}"
...
class TestSpecificManufacturers:
    ...
    def test_abaflex_parsing(self, filaments):
        ...
        transparent = [f for f in abaflex
                      if 'PETG' in f['material']
                      and f['color'] == 'Transparent']
